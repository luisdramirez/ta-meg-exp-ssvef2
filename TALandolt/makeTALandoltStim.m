function makeTALandoltStim(run)
...
%% file i/o
stimDir = 'stimuli';
stimFile = sprintf('taLandolt%d', run);
...
exoTargetSOA = 0.1; % (s) % SOA between exo stim and target
...
blockNames = {'blank','slow-left'}; % slow-left
attBlockNames = {'no-att','att-right'}; % att-right
exoBlockNames = {'no-exo','pres-pres','pres-abs','abs-pres','abs-abs'};
cueBlockNames = {'no-cue','1-1','1-2','2-1','2-2'}; % 2-1 = cueT2,postcueT1
targetBlockNames = {'no-targ','left-left','left-right','right-left','right-right'}; % direction of the Landolt gap
[blockOrder, attBlockOrder, targetBlockOrder, cueBlockOrder, exoBlockOrder] = ...
    block_gen_exo(blockNames, attBlockNames, targetBlockNames, cueBlockNames, exoBlockNames);
...
p = v2struct(...
    ...
    targetBlockNames, targetBlockOrder, exoBlockNames, exoBlockOrder, ...
...
for iBlock = 1:nBlocks
    if ~strcmp(blockNames{blockOrder(iBlock)},'blank')
        ...
        exoTimes = targetTimes - exoTargetSOA;
        
        % eliminate "absent" targets, depending on condition
        exoBlockName = exoBlockNames{exoBlockOrder(iBlock)};
        switch exoBlockName
            case 'pres-pres'
                exoAbsTimes = [];
            case 'pres-abs'
                exoAbsTimes = exoTimes(2);
                exoTimes(2) = [];
            case 'abs-pres'
                exoAbsTimes = exoTimes(1);
                exoTimes(1) = [];
            case 'abs-abs'
                exoAbsTimes = exoTimes;
                exoTimes = [];
            otherwise
                error('exoBlockName not recognized')
        end
        ...
        % exo present
        exoTimes = exoTimes + blockStartTimes(iBlock) + targetLeadTime;
        exoStartTimes = [exoStartTimes; exoTimes'];
        
        % exo absent
        exoAbsTimes = exoAbsTimes + blockStartTimes(iBlock) + targetLeadTime;
        exoAbsStartTimes = [exoAbsStartTimes; exoAbsTimes'];
...
exoEndTimes = exoStartTimes + exoDur;
nExos = numel(exoStartTimes);
% all these exos are on the right
exoSides = ones(1,nExos)*2;

% exo absent is treated in the same way
exoAbsEndTimes = exoAbsStartTimes + exoDur;
nAbsExos = numel(exoAbsStartTimes);
exoAbsSides = ones(1,nAbsExos)*2;
...
targetOn = 0;
exoIdx = 1; % exo present
exoOn = 0;
exoAbsIdx = 1; % exo absent
exoAbsOn = 0;
cueIdx = 1;
for iFrame = 1:numel(seqtiming)
    ...
    %% determine if exo is on
    % turn it on when it is time and leave on until time to turn off
    if exoIdx <= nExos && exoStartTimes(exoIdx)-time < 1/refrate
        exoOn = 1;
    end
    if exoIdx <= nExos && exoEndTimes(exoIdx)-time < 1/refrate
        exoOn = 0;
        exoIdx = exoIdx + 1;
    end
    if exoOn
        if exoSides(exoIdx)==1
            c1 = 2;
            c2 = 1;
            exoOnSeq(iFrame,1) = 1;
        elseif exoSides(exoIdx)==2
            c1 = 1;
            c2 = 2;
            exoOnSeq(iFrame,1) = 2;
        end
    ...
    %% determine if an absent exo is "on" (if it is time for that target)
    % treat it the same way as real exos, in order to determine
    % triggers, but do not presnet the exo
    if exoAbsIdx <= nAbsExos && exoAbsStartTimes(exoAbsIdx)-time < 1/refrate
        exoAbsOn = 1;
    end
    if exoAbsIdx <= nAbsExos && exoAbsEndTimes(exoAbsIdx)-time < 1/refrate
        exoAbsOn = 0;
        exoAbsIdx = exoAbsIdx + 1;
    end
    if exoAbsOn
        if exoAbsSides(exoAbsIdx)==1
            exoAbsOnSeq(iFrame,1) = 1;
        elseif exoSides(exoAbsIdx)==2
            exoAbsOnSeq(iFrame,1) = 2;
        end
    else
        exoAbsOnSeq(iFrame,1) = 0;
    end
    ...
    if ~strcmp(cueBlock, 'no-cue') && ...
        ...
        switch targetBlockNames{targetBlockOrder(blockIdx)}
            case 'left-left'
                correctResponse = 1; % left
            case 'left-right'
                if responseCue==1
                    correctResponse = 1; % left
                else
                    correctResponse = 2; % right
                end
            case 'right-left'
                if responseCue==1
                    correctResponse = 2; % right
        ...
        case 'conditionID'
            ...
            elseif targetOnSeq(iFrame)~=0
                ...
                if targetOnSeq(iFrame)==1 && targetOnSeq(iFrame-1)==0
                    trig = NaN; % target on left (never happens)
                elseif targetOnSeq(iFrame)==2 && targetOnSeq(iFrame-1)==0
                    trig = 6; % target on right
                else
                    trig = NaN;
                end
            elseif exoOnSeq(iFrame)~=0
                % triger for target side, only on first target frame
                if exoOnSeq(iFrame)==1 && exoOnSeq(iFrame-1)==0
                    trig = NaN; % exo on left (never happens)
                elseif exoOnSeq(iFrame)==2 && exoOnSeq(iFrame-1)==0
                    trig = 5; % exo on right
                else
                    trig = NaN;
                end              
            elseif exoAbsOnSeq(iFrame)~=0
                % triger for target side, only on first target frame
                if exoAbsOnSeq(iFrame)==1 && exoAbsOnSeq(iFrame-1)==0
                    trig = NaN; % exo abs on left (never happens)
                elseif exoAbsOnSeq(iFrame)==2 && exoAbsOnSeq(iFrame-1)==0
                    trig = 4; % exo abs on right
                else
                    trig = NaN;
                end
            elseif cueSeq(iFrame)~=0
                lastTrig = trigSeq(find(trigSeq>0,1,'last'));
                if lastTrig == 1 % new block
                    trig = 2; % pre-cue
                else
                    trig = 3; % post-cue
                end
...
nSets = 4;
nPerSet = numel(trigSeq)/nSets;
for i = 1:nSets
    idx = (1:nPerSet)+nPerSet*(i-1);
    f(i) = figure;
    subplot(4,1,1)
    plot(seqtiming(idx),targetOnSeq(idx))
    subplot(4,1,2)
    hold on
    plot(seqtiming(idx),exoOnSeq(idx))
    plot(seqtiming(idx),exoAbsOnSeq(idx),'g')
    subplot(4,1,3)
    plot(seqtiming(idx),trigSeq(idx))
    subplot(4,1,4)
    plot(seqtiming(idx),keyCodeSeq(idx))
end
...
% store in order structure %%% store exo
